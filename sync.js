...
(function (global) {
  ...
  function mergeById(localList, remoteList) {
    ...
    (remoteList || []).forEach(remote => {
      ...
      if (local.synced !== false) {
        byId.set(id, { ...local, ...remote, synced: true });
        updated++;
      }
    });
  ...
  async function pullFromCloud(options = {}) {
    ...
    if (!unit) {
      if (!silent) setSyncUI('err', 'No unit number');
    ...
    try {
      ...
      const url = driver.syncUrl + sep + 'type=driver&unit=' + encodeURIComponent(unit) + '&ts=' + Date.now();
      ...
      const loadMerge = mergeById(_get.loads(), data.loads || []);
      const ptiMerge  = mergeById(_get.ptiLog(), data.ptiLog || []);
