...
assert.equal(context.CrewBIQOwnerSnapshots.version, '0.1.0');
...
// A cloud restore containing the deleted row cannot resurrect it while pending.
...
assert.deepEqual(
  ...
  ['deductionTemplates', 'expenses', 'weeklyDeductions'],
);
...
// A failed sync retains pending deletion for retry after app restart.
responseOk = false;
context.saveExpenses([]);
await context.fetch('https://crewbiq-orchestrator-production.up.railway.app/v1/sync/pwa', {
  method: 'POST',
  body: JSON.stringify({ type: 'driver_report', driver: JSON.parse(storage.get('fiqD_driver')), ownerData: {} }),
});
assert.deepEqual(context.CrewBIQOwnerSnapshots.loadPending().expenses.value, []);
