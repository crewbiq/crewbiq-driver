/**
 * CrewBIQ Loads Module v0.4.1 — Date Display / Report Scope Patch
...
(function (global) {
  ...
  // Internal data may come as YYYY-MM-DD or full ISO (2026-05-01T05:00:00.000Z).
  // UI/PDF/share must never show raw ISO or Invalid Date.
  ...
  function recalcLoadsFrom(effectiveDate, rates) {
    ...
    const updated = _get.loads().map(x => {
      const pickupDate = normalizeDateValue(x.pickup || x.date || '');
      const effDate = normalizeDateValue(effectiveDate);
      if (pickupDate < effDate) return x;
      count++;
  ...
  function getExportLoads() {
    ...
    return _get.loads().filter(x => {
      const d = normalizeDateValue(x.pickup || x.date || '');
      return d && d >= nf && d <= nt;
    });
  }
  ...
  function fmtDateRangeFromLoads(rLoads) {
    ...
    const dates = rLoads.map(x => normalizeDateValue(x.pickup || x.date || '')).filter(Boolean).sort();
  ...
  function buildReportHTML() {
    ...
    // Categorize. IMPORTANT: older loads may have no status; treat them as active
    // so PDF reports never look empty after migrations.
    const statusOf = (x) => x.status || 'active';
    const paidLoads     = allLoads.filter(x => statusOf(x) === 'success' || wonIds.has(x.loadId));
    ...
    // KPI from paid + active + adj
    const kpiLoads = [...paidLoads, ...activeLoads, ...adjLoads];
...
<html><head>
  ...
  function renderHome() {
    ...
    document.getElementById('homeWeekSub').textContent  = `${isCPM ? '' : 'gross ' + _fmt(gross) + ' · '}${(lmi + dmi).toLocaleString()} mi · ${wkLoads.length} loads${disputedGross > 0 ? ' · ⚖️ ' + _fmt(disputedGross) + ' disputed' : ''}`;
      ...
      return `<div class="item" style="border-left:3px solid ${borderColor}">
        <div class="item-inner">
          <div>
            ...
            <div class="muted">${displayLoadDateRange(x)}</div>
  ...
  function renderStats() {
    ...
    const wkMap = {};
    loads.forEach(x => {
      const wk = _getWeekKey(normalizeDateValue(x.pickup || x.date || ''));
      if (!wk) return;
  ...
  console.info('[CrewBIQ Loads] v0.4.0 loaded');
