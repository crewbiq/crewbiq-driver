...
(function (global) {
  ...
  function buildReportText() {
    ...
    const { from: pFrom, to: pTo } = getPeriodDates();
    const dateRange = rLoads.length ? fmtDateRangeFromLoads(rLoads) : fmtDateRange(pFrom, pTo);
  ...
  function buildReportHTML() {
    ...
    // Categorize
    const paidLoads     = allLoads.filter(x => x.status === 'success' || wonIds.has(x.loadId));
    const activeLoads   = allLoads.filter(x => x.status === 'active' && !pendingIds.has(x.loadId) && !wonIds.has(x.loadId));
    const adjLoads      = allLoads.filter(x => x.status === 'adj' && !wonIds.has(x.loadId));
    const disputedLoads = allLoads.filter(x => pendingIds.has(x.loadId) || (x.status === 'disputed' && !wonIds.has(x.loadId)));
    const cancelLoads   = allLoads.filter(x => x.status === 'cancel' || lostIds.has(x.loadId));

    const period    = getPeriodLabel();
    const { from: pFrom, to: pTo } = getPeriodDates();
    const dateRange = allLoads.length
      ? fmtDateRangeFromLoads(allLoads)
      : fmtDateRange(pFrom, pTo);
    ...
    function makeSection(title, loads, hColor) {
      ...
      const rows  = loads.map((x,i) => {
        ...
        const statusLabel = x.status==='success'?'\u2705 Success':x.status==='adj'?`\uD83D\uDD27 Adj`:x.status==='cancel'?'\u274C Cancelled':'\u{1F535} Active';
